...
/// https://tools.ietf.org/html/rfc6902
 class JSONPatch: Codable {
    
    /// The mimetype for json-patch
     static let mimetype = "application/json-patch+json"

    /// Options given to the patch process.
     enum ApplyOption: Equatable {
    ...
    ///   - operations: An array of operations.
     init(operations: [JSONPatch.Operation]) {
    ...
    ///   - jsonArray: An array obtained from JSONSerialization containing json-patch operations.
     convenience init(jsonArray: NSArray) throws {
...
extension JSONPatch.Operation {
    ...
    ///   - index: The index this operation occurs at within the json-patch document.
     init(jsonObject: NSDictionary, index: Int = 0) throws {
...
extension JSONPatch.Operation: Equatable {
    ...
    /// - Returns: true is the lhs is equal to the rhs.
     static func == (lhs: JSONPatch.Operation, rhs: JSONPatch.Operation) -> Bool {
        switch (lhs, rhs) {
